/**
 * Tests for the anchors list-route error boundary.
 *
 * Verifies that:
...
import AnchorsError from "./error";
...
describe("AnchorsError boundary", () => {
  ...
  it("renders the 'Could not load anchors' title", () => {
    render(<AnchorsError error={new Error("fetch failed")} reset={() => {}} />);
    expect(screen.getByText("Could not load anchors")).toBeInTheDocument();
  });

  it("calls the reset callback when 'Try again' is clicked", () => {
    const reset = vi.fn();
    render(<AnchorsError error={new Error("fetch failed")} reset={reset} />);

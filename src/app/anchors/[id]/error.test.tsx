/**
 * Tests for the anchor detail ([id]) route error boundary.
 *
 * Verifies that:
...
import AnchorDetailError from "./error";
...
describe("AnchorDetailError boundary", () => {
  ...
  it("renders the 'Could not load this anchor' title", () => {
    render(
      <AnchorDetailError error={new Error("not found")} reset={() => {}} />,
    );
    expect(screen.getByText("Could not load this anchor")).toBeInTheDocument();
  });

  it("calls the reset callback when 'Try again' is clicked", () => {
    const reset = vi.fn();
    render(<AnchorDetailError error={new Error("not found")} reset={reset} />);
